...
is to learn co-regulation netwok in which gene regulation is modeled by
...
matrix with unique column names and row names containing sample and gene names
respectively 
...
_discretizeExpressionData_. This function can be used to discretize
gene expression with or without a set of reference samples, 
...
Although LICORN is an inference algorithm which is particularly
...
select local gene regulatory network. 
...
adjusted R2 in the case of _hLICORN_, and each of the score given by the
external datasets are then merged. Two methods are proposed to do so, both proposed by the _modEncode_ consortium.
...
of given transcription regulator in a given sample. This measure,
...
webpage. The objective of the embeded visualization tool is to navigate through 
...
gene ({TF1,TF2,...} -> Gene, inferred using the LICORN algorithm embedded in the package). From
...
The minimum requirement to launch the visualization tool is a _coregnet_
...
![View of the shiny Web page](fig/wholeApp.png)
